...
export default function MiniGame({ isOpen, setIsOpen }: MiniGameProps) {
  ...
  // Dynamic game speed increases with score
  const GAME_SPEED = BASE_GAME_SPEED + Math.floor(score / 20) * 0.5;
  ...
  useEffect(() => {
    ...
    if (score > 0 && score % 25 === 0 && score !== milestoneScore) {
  ...
  const jump = useCallback(() => {
    if (!gameOver && isPlaying && dragonY === 0 && !isJumping) {
      ...
      createParticles(DRAGON_X + DRAGON_SIZE / 2, GROUND_Y, "#fbbf24", 8);
    }
  }, [gameOver, isPlaying, dragonY, isJumping, JUMP_STRENGTH, createParticles, DRAGON_X, DRAGON_SIZE, GROUND_Y]);
        ...
        prev
          .map((volcano) => {
            ...
            if (
              ...
              createParticles(
                DRAGON_X + DRAGON_SIZE / 2,
                GROUND_Y - dragonY - DRAGON_SIZE / 2,
                "#ef4444",
                20
              );
              setGameOver(true);
            ...
            >
              ...
              {volcanoes.map((volcano, i) => (
                ...
                >
                  <div className="relative w-full h-full">
                    {/* Volcano body */}
                    <div
                      className="absolute bottom-0 bg-gradient-to-t from-red-700 via-red-600 to-orange-500 shadow-lg"
                      style={{
                        width: "100%",
                        height: "100%",
                    ...
                    />
                    {/* Lava with glow */}
                    <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-4 bg-yellow-400 rounded-full animate-pulse shadow-lg shadow-orange-500/50" />
                    {/* Smoke particles */}
                    <motion.div
                      className="absolute -top-2 left-1/2 -translate-x-1/2 w-2 h-2 bg-gray-400/60 rounded-full"
                      animate={{
                        y: [-10, -30],
                        opacity: [0.6, 0],
                        scale: [1, 1.5],
              ...
              <AnimatePresence>
                {showMilestone && (
                  ...
                  >
                    <div className="bg-gradient-to-r from-purple-500 to-pink-500 px-8 py-4 rounded-2xl shadow-2xl border-2 border-white/30">
                      <div className="text-center">
                        <div className="text-4xl mb-2">🎉</div>
                        <div className="text-3xl font-bold text-white">
                          {milestoneScore} Points!
                        </div>
              ...
              {score >= 20 && (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="absolute top-16 right-4 bg-black/50 backdrop-blur-sm px-3 py-1.5 rounded-lg text-xs text-yellow-400 font-semibold"
                >
                  Speed: {GAME_SPEED.toFixed(1)}x
                </motion.div>
              )}
